...
function formatInverterOption(result) {
  ...
  const labelParts = [
    ...
    idInfo.firmwareVersion ? `FW ${idInfo.firmwareVersion}` : undefined,
...
function getDiscoveryCandidates(options = {}) {
  ...
  if (validation.valid) {
    const octets = validation.octets ?? [];

    return getIpv4CandidatesFromSubnet(`${octets.slice(0, 3).join(".")}.0/24`);
  }
  ...
  for (const entries of Object.values(os.networkInterfaces())) {
    for (const entry of entries ?? []) {
      ...
      if (entry.family === "IPv4" && !entry.internal && validation.valid) {
        ...
        subnets.add(`${octets.slice(0, 3).join(".")}.0/24`);
      }
    }
  }
