...
// Range a writable register accepts. GoodWe documents no writable registers, so
// the bounds are the safety net: a value outside them is never sent.
interface WritableRange {
  min: number;
  max: number;
}
...
const registerGroups: Record<string, RegisterGroup> = {
  ...
  settingsBattery: {
    ...
    entries: [
      ...
      entry(
        45352,
        "Settings.Battery.ChargeVoltage",
      ...
      entry(
        45353,
        "Settings.Battery.ChargeCurrent",
      ...
      entry(
        45354,
        "Settings.Battery.DischargeVoltage",
        "Battery.DischargeVoltage",
        TYPE.U16,
        { scale: 10, unit: "V" },
      ),
      entry(
        45355,
        "Settings.Battery.DischargeCurrent",
      ...
      entry(
        45356,
        "Settings.Battery.DischargeDepth",
        "Battery.DischargeDepth",
        TYPE.U16,
        { unit: "%", role: "value.battery" },
      ),
      entry(
        45357,
        "Settings.Battery.DischargeVoltageOffline",
        "Battery.DischargeVoltageOffline",
        TYPE.U16,
        { scale: 10, unit: "V" },
      ),
      entry(
        45358,
        "Settings.Battery.DischargeDepthOffline",
        "Battery.DischargeDepthOffline",
        TYPE.U16,
        { unit: "%", role: "value.battery" },
      ),
    ],
...
const writableEntries: ReadonlyMap<string, RegisterEntry> = new Map(
  Object.values(registerGroups).flatMap((group) =>
    group.entries
      .filter((item) => item.writable !== undefined)
      .map((item): [string, RegisterEntry] => [item.state, item]),
  ),
);
 ...
 */
function registerGroupOfState(state: string): string {
...
export type { RegisterEntry, RegisterGroup, WritableRange };
export {
